...
interface ArrayDisplay<Field extends FormBody.AnyArray>
  ...
  Element: FormDisplay.AnyFieldDisplay<Field['field']>;
}
...
interface RawDisplay<Field extends FormBody.AnyRaw>
  extends FormFramework.MakeRaw<Field> {}
...
type FormDisplay<Body extends FormBody.Any> = {
  [key in keyof Body['fields']]: FormDisplay.AnyFieldDisplay<
    Body['fields'][key]
  >;
} & {
...
const addControls = <T>(component: T, path: Path) =>
  Effect.gen(function* () {
    const { makeFieldControls } = yield* FormFramework;
...
export declare namespace FormDisplay {
